...
//we are gonna assume that if we see no barrels (aka IDs of 2), then the board is empty therefore we won
//otherwise, the moment we see a ID == 2, that means there still exists at least one barrel and therefore, we havn't won yet
bool
Grid::isDone()
{
	for(int y = 0; y < nCols; y++){
		for(int x = 0; x < nRows; x++){
			if (this->data[y].data[x].getID() == 2){
				return false; //found an uncollected barrel
			}
		}
	}
