...
4. Ghosts move to random intersections/turn nodes
5. There are 4 levels that automatically load each time you pass the current level but, reset back to level 1 when you run out of lives
1. Ghosts travel using A* towards random turn nodes or to where the player is located
...
7. Files inside "screenshots and video" folder


Known Bugs:

	Sometimes a ghost walks through a wall (probably fixed by now)
	Not all memory is freed when changing levels
